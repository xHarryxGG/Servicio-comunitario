...
def registrarEstudiante(request):
    ...
    Estudiante = estudiante.objects.create(cedula = cedula, nombre = nombre, apellido = apellido, año = año, seccion = seccion)
...
def registrarAcademico(request):
    ...
    Academico = academico.objects.create(cedula = cedula, nombre = nombre, apellido = apellido)
...
def registrarLibro(request):
    ...
    Libro = libro.objects.create(codigolibro = codigolibro, titulo = titulo, autor = autor, materia = materia, año = año, cantidad = cantidad, cantidadPres = cantidadPres)
...
def homePrestamos(request):
    ...
    academicos = academico.objects.all().order_by('nombre')
    administrativos = administrativo.objects.all().order_by('nombre')
...
def registrarPrestamo(request):
    ...
    if estudiante.objects.filter(cedula=cedula).exists():
        usuarioE = estudiante.objects.get(cedula=cedula)
    ...
    if int(usuarioE.prestamos) > 3:
        messages.error(request, "ERROR:  No se pudo eliminar porque el usuario ya tiene 4 prestamos")

    else:
        Prestamo = prestamo.objects.create(codigo = codigo, tipoUsuario = tipoUsuario, tipoPrestamo = tipoPrestamo, codigolibro = codigolibro, titulo = titulo, autor = autor, cedula = cedula, nombre = nombre, apellido = apellido, fechaInicial = fechaInicial, fechaFinal = fechaFinal, bibliotecaria = bibliotecaria, observaciones = observaciones)
